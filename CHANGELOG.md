...
- **舊的角標偏好 `'2'`（全部）遷移為「勳章牆 + 手動添加 + 特別關注 + 喜歡」，不含「其他全部關注」。** 這是一次刻意的、無聲的設定變更。`'2'` 在勳章牆時代的實際語意是「我監控的那些人」（約 90 人），v3.0 把母體換成 1769 人的完整關注列表之後，這個碼的含義才漂成「所有關注」——那正是本次要移除的行為。其餘偏好碼（`'0'` / `'1'` / `'3'` / `'4'`）皆一對一忠實遷移。升級後可在設定面板隨時勾回。
