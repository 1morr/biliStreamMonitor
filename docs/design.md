...
- 全關注模式每週期：`following?page_size=29` 翻頁直到某頁無 `live_status=1`（約 120 人直播中 ≈ 4–5 次請求）；頁面資料已含 title/cover/uname/live_status，**無需額外補查**。
...
| 其餘關注（關注列表中的其餘） | `following` 翻頁 × 約 5 |
...
關注數上千時列完要翻 60+ 頁——不可行。因此「全部」= 直播中的關注 + 手動挑選的那組（含離線）。
