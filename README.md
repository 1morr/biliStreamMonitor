...
v3.0 起，扩展支持 **全关注模式**（默认）与 **勋章墙模式** 双模式切换：全关注模式覆盖你关注的所有主播的开播状态，修复了旧版仅依赖粉丝勋章墙导致的大量漏报（实测勋章墙只覆盖约 12% 的关注开播）。

## ✨ 主要功能

*   **双监控模式**：
    *   **全关注模式（默认）**：基于关注列表翻页获取全部关注主播的直播状态，不漏报。
    *   **勋章墙模式**：仅监控你拥有粉丝勋章的主播，轻量省电。
...
*   **自定义房间**：监控未关注的主播——在弹窗顶栏粘贴房间号即可添加，状态通过批量 API 一次刷新。
*   **高度自定义**：
...
### 2. 选择监控模式
在设置面板的 **Monitor Mode** 中选择：
*   **Following（全关注，默认）**：监控你关注的所有主播。每个周期对关注列表翻页数次（直播中者排最前，遇到无直播中的页即停，上限 20 页）。
*   **Medal Wall（勋章墙）**：仅监控你拥有粉丝勋章的主播，每周期仅 1 次请求。适合只在意勋章主播的用户。

两种模式都会排除“轮播/回放”状态的房间，只认真实开播。
...
### 4. 添加自定义房间
想监控但未关注的主播，在弹窗 **顶栏输入框** 粘贴房间号（或直播间 URL）并点击 `+` 即可。自定义房间的直播状态每个周期通过批量接口（`get_status_info_by_uids`，每批 ≤200 个 uid）一次刷新，不产生逐房间的请求。已添加的房间可在设置面板的 **Custom Rooms** 中管理删除。
...
*   **调用量模型**：默认 60 秒周期下，全关注模式每周期 = 关注列表翻页数次（实测 116 个直播中约 4–5 次）+ 自定义房间 1 次批量请求，合计约 5–6 次/分钟；勋章墙模式仅约 2 次/分钟。弹窗打开本身零 API 调用（只读本地存储）。该量级远低于已知的 B 站风控触发区间（社区安全锚点为 30–60 秒轮询）。
...
*   [`docs/design.md`](docs/design.md)：v3.0 设计决策记录——双模式、自定义房间批次化、i18n、目录结构等方案的选型过程。
...
*   勋章墙模式下，只有你拥有粉丝勋章的主播才会出现在列表中；如需完整覆盖请使用默认的全关注模式。
